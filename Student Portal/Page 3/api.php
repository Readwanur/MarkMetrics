...
if ($action === 'withdraw_course') {
    ...
    $verify_stmt = $conn->prepare("
        SELECT e.enrollment_id, e.course_code, e.status, c.course_name
        FROM enrollments e
        JOIN courses c ON e.course_code = c.course_code
    ...
    $update_stmt = $conn->prepare("UPDATE enrollments SET status = 'Dropped', grade = 'W', points = 0.00 WHERE enrollment_id = ?");
    $update_stmt->bind_param("i", $enrollment_id);

    if ($update_stmt->execute()) {
        // Log the withdrawal
        $log_desc = "Student $user_id withdrew from {$enrollment['course_code']} ({$enrollment['course_name']})";
        if (!empty($reason)) {
            $log_desc .= ". Reason: $reason";
        }
        $log_stmt = $conn->prepare("INSERT INTO audit_logs (user_id, action_type, description, target_entity_type, target_entity_id) VALUES (?, 'Course Withdrawal', ?, 'enrollments', ?)");
        $eid_str = (string)$enrollment_id;
        $log_stmt->bind_param("sss", $user_id, $log_desc, $eid_str);
        $log_stmt->execute();

        echo json_encode([
            'success' => true,
            'message' => "Successfully withdrew from {$enrollment['course_name']}"
        ]);
    } else {
        http_response_code(500);
